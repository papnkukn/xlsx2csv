    ...
    xlsx2json(file, options, function(error, result) {
      ...
      try {
        for (var i = 0; i < result.worksheets.length; i++) {
          ...
          if (options.verbose) {
            console.log("Worksheet has " + (end.row - start.row + 1) + " rows and " + (end.column - start.column + 1) + " columns: " + range);
          }
            ...
            if (pos.row <= end.row && pos.column <= end.column) {     
              if (item.formula || item.value) {
                if (options.data == "formula" && item.formula) {
                  table[pos.row - 1][pos.column - 1] = item.formula; 
                }
                else if (item.value) {
                  table[pos.row - 1][pos.column - 1] = options.data == "display" && typeof item.display != "undefined" ? item.display : item.value;
                }
              }
            }
          }
